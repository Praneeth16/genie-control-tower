/** The Ask lane: question -> routing -> per-domain Genie answers -> one fused answer -> a proposable
 *  action.
 *
 *  Two things are deliberately not hidden. The SQL each Genie Agent wrote is one click away, because
 *  an analyst who cannot see the query cannot sign off on the number. And the per-leg timing is shown,
...
const EXAMPLES = [
  "Microfinance delinquency in Solapur jumped since March. Are those customers also complaining more, and what should we do?",
  "Why did payment-systems complaints spike in June 2026?",
  "Which RMs are paid a full incentive while failing the product-mix quality gate?",
  "Which recovery agents are contacting borrowers outside permitted hours?",
];
...
export function Ask({ onActed }: { onActed: () => void }) {
  ...
  // The proposed action, its editable schedule, and the verdict once proposed.
  ...
  async function ask() {
    ...
    try {
      ...
      setResult(r);
      // Default the schedule to a compliant slot so the happy path works, while leaving it editable —
      // changing it to an evening slot is how the conduct control gets demonstrated.
      const d = new Date();
      d.setHours(15, 0, 0, 0);
      ...
      <Card className="border-border">
        <CardContent className="space-y-3 p-4">
          <Textarea
            ...
            placeholder="Ask across collections, grievance and RM performance…"
            rows={2}
          ...
          />
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={ask} disabled={busy || !question.trim()} size="sm">
              {busy ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> Asking three agents…
                </>
              ) : (
                <>
            ...
            <span className="text-[11px] text-muted-foreground">
              These are reasoning answers over 268,000 rows — expect 15–45 seconds.
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="rounded border border-border bg-secondary/50 px-2 py-1 text-left text-[11px] text-muted-foreground hover:border-primary/40 hover:text-foreground"
              >
                {ex.length > 66 ? ex.slice(0, 66) + "…" : ex}
              </button>
            ))}
          </div>
        </CardContent>
            ...
            <CardContent className="space-y-2 p-4">
              ...
              {result.router_fallback && (
                <div className="rounded border border-warning/40 bg-warning/10 px-2 py-1 text-[11px] text-warning">
                  The router did not return a usable decision, so a single domain was consulted rather
                  than guessing across all three.
                </div>
              )}
